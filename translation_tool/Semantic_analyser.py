...
class Semantic_analyser(object):
    ...
    def initialise(self):
        ...
        # {AST_TYPE.INT_DECL: lambda self: self.analyze_1 }
        # self.node_type_lookup = {AST_TYPE.INT_DECL: self.analyse_int_decl}
    ...
    def expr_type_is(self, expression):
        ...
        for i, expr in enumerate(expression.expressions):
            if expr.node_type == AST_TYPE.INT_VAL:
                ...
                expr_types["OPERAND_" + str(len(expr_types))] = AST_TYPE.SEQ_VAL
            elif expr.node_type == AST_TYPE.BIT_VAL:
                expr_types["OPERAND_" + str(len(expr_types))] = AST_TYPE.BIT_VAL
            elif expr.node_type == AST_TYPE.SHIFT_OP or expr.node_type == AST_TYPE.ARITH_OP:
