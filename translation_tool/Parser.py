...
class TestASTTree(unittest.TestCase):
    ...
    def test_bit_decl(self):
        ...
        assert_equals(par.AST.tree[0].node_type,  AST_TYPE.BIT_DECL)
        assert_equals(par.AST.tree[1].node_type, AST_TYPE.BIT_DECL)
        assert_equals(par.AST.tree[1].value.node_type, AST_TYPE.EXPR)
    ...
    def test_int_decl(self):
        ...
        assert_equals(par.AST.tree[0].node_type,  AST_TYPE.INT_DECL)
        assert_equals(par.AST.tree[1].node_type,  AST_TYPE.INT_DECL)
        assert_equals(par.AST.tree[2].node_type,  AST_TYPE.INT_DECL)
        ...
        assert_equals(par.AST.tree[0].value.expressions[0].expressions[1].operator,  "*")
        assert_equals(par.AST.tree[0].value.expressions[0].expressions[2].node_type, AST_TYPE.CAST)
    ...
    def test_bs_decl(self):
        par = Parser()
        assert_equals(par.parse_test_unit("@Int(10) varName;")[1], True)
        assert_equals(par.AST.tree[0].node_type, AST_TYPE.BS_INT_DECL)
        assert_equals(par.parse_test_unit("Int(10) varName;")[1], True)
        assert_equals(par.AST.tree[1].node_type, AST_TYPE.INT_DECL)
        assert_equals(par.parse_test_unit("@Int(10)[10] varName;")[1], True)
        assert_equals(par.AST.tree[2].node_type, AST_TYPE.BS_SEQ_INT_DECL)
        assert_equals(par.parse_test_unit("Int(10)[10] varName;")[1], True)
        assert_equals(par.AST.tree[3].node_type, AST_TYPE.SEQ_INT_DECL)
        assert_equals(par.parse_test_unit("Bit[10] varName;")[1], True)
        assert_equals(par.AST.tree[4].node_type, AST_TYPE.SEQ_BIT_DECL)
    ...
    def test_int_seq_decl(self):
        ...
        assert_equals(par.AST.tree[0].value.expressions[0].expressions[0].node_type, AST_TYPE.SEQ_VAL)
        ...
        assert_equals(par.AST.tree[6].value.expressions[0].indices[0].expressions[0].expressions[0].value, "1")
        assert_equals(par.AST.tree[6].value.expressions[0].indices[0].expressions[0].expressions[1].operator, "*")
        assert_equals(par.AST.tree[6].value.expressions[0].indices[0].expressions[0].expressions[2].value, "4")
    ...
    def test_bit_seq_decl(self):
        ...
        assert_equals(par.parse_test_unit("Bit[1] b;")[1], True)
        assert_equals(par.AST.tree[0].node_type, AST_TYPE.SEQ_BIT_DECL)
    ...
    def test_id_set(self):
        ...
        assert_equals(par.AST.tree[2].elements.expressions[0].expressions[0].value, "1")
        assert_equals(par.AST.tree[2].elements.expressions[1].expressions[0].value, "2")
        assert_equals(par.AST.tree[2].elements.expressions[3].expressions[0].start.expressions[0].value, "4")
        assert_equals(par.AST.tree[2].value.expressions[0].target.expressions[0].expressions[2].node_type, AST_TYPE.FUNCTION_CALL)
    ...
    def test_return_stmt(self):
        ...
        assert_equals(par.AST.tree[0].node_type, AST_TYPE.RETURN_STMT)
    ...
    def test_for_loop(self):
        ...
        assert_equals(par.parse_test_unit("for(;;) { }")[1], True)
        assert_equals(par.AST.tree[0].node_type, AST_TYPE.FOR_LOOP)
        assert_equals(par.AST.tree[1].node_type, AST_TYPE.FOR_LOOP)
        assert_equals(par.AST.tree[0].terminator[0].expressions[0].expressions[1].operator, "&&")
        assert_equals(par.AST.tree[0].body[0].node_type, AST_TYPE.ID_SET)
    ...
    def test_if_stmt(self):
        ...
        assert_equals(par.AST.tree[0].node_type, AST_TYPE.IF_STMT)
        assert_equals(par.AST.tree[1].node_type, AST_TYPE.IF_STMT)
...
class TestSemanticAnalysisTree(unittest.TestCase):
    ...
    def test_if_stmt(self):
        par = Parser()
        assert_equals(par.analyse_tree_test(par.parse_test_AST_semantic("Int(4) a = 4; if(4 > 4) { a = 10; }")), True)

    def test_for_loop(self):
        par = Parser()
        assert_equals(par.analyse_tree_test(par.parse_test_AST_semantic("for(Int(10) i = 5, Bit b = False; b != True; i = i + 5) { b = True; }")), True)
        par = Parser()
        assert_equals(par.analyse_tree_test(par.parse_test_AST_semantic("for(Int(10) i = 5, Bit b = False; b != True; i = i + 5) { b = True; } b = True;")), False)  #b is out of scope!
        par = Parser()
    ...
    def test_func_decl(self):
        par = Parser()
        assert_equals(par.analyse_tree_test(par.parse_test_AST_semantic("Int(8) test_func(Int(8) a) { Int(10) d = 1;  Bit b = False; }")), True)
        assert_equals(par.semantic_analyser.sym_table.f_table['test_func']['return_type'], AST_TYPE.INT_VAL)
    ...
    def test_func_call(self):
        ...
        assert_equals(par.analyse_tree_test(par.parse_test_AST_semantic("Bit test_func1(Int(10) a) { a = a + 10; }\
                                                                            Int(10) test_func(Int(10) a) { Int(10) d = test_func(test_func1(10)) + 20; }")), False)
        par = Parser()
...
#         par = Parser()
#         assert_equals(par.analyse_tree_test(par.parse_test_AST_semantic("Int(8) a = 1; @Int(8)[8] state = [a,2,3,4,5,0x6,0b0111,8];")), True)
#         assert_equals(par.semantic_analyser.sym_table.id("state")["value"], ['0', '0', '0', '0', '1', '30', '102', '170'])
#         par = Parser()
#         assert_equals(par.analyse_tree_test(par.parse_test_AST_semantic("Int(8) a = 1; @Int(8)[8] state;")), True)
#         par = Parser()
#         assert_equals(par.analyse_tree_test(par.parse_test_AST_semantic("@Int(8)[8] state = [1,2,3,4,5,6,7,8];\
#                                                                           Int(8) output;\
#                                                                           Int(8) input;")), True)
#         par = Parser()
#         assert_equals(par.analyse_tree_test(par.parse_test_AST_semantic("@Int(8)[8] state = [1,2,3,4,5,6,7,8];\
#                                                                           Int(8) output;\
#                                                                           Int(8) input;\
#                                                                           for(Int(5) i = 0,Int(5) z = 0, Bit d = True; i < 32 && i > 20; i = i + 1) {\
#                                                                           }")), True)
