...
return ((~C & B & ~A) | (D & C & A) | (D & ~B & A) | (~D & B & ~A) | (~D & ~C & B) | (D & ~C & ~B));
}
uint32_t present_2(uint32_t A, uint32_t B, uint32_t C, uint32_t D) {
return ((~D & C & B & A) | (~C & B & ~A) | (D & C & ~B) | (D & ~B & A) | (~C & ~B & A) | (~D & ~C & ~B));
}
uint32_t present_3(uint32_t A, uint32_t B, uint32_t C, uint32_t D) {
return ((~C & B & A) | (~D & C & ~A) | (~D & B & A) | (~D & ~B & ~A) | (D & ~C & B) | (~D & C & B) | (D & ~C & A));
}
...
uint32_t temp_23_rnge[(((i * 4) + 3) - (i * 4)) + 1];
extract_bs_range(temp_23_rnge, state, (i * 4), ((i * 4) + 3));
present(temp_23_rnge);
if((i * 4) >= 64){
fprintf(stderr, "Index out of bounds for selection on state\n");
exit(1);
}
...
uint8_t temp_33_init = 0;
uint32_t temp_34_rnge_size = 0;
temp_34_rnge_size = (((i * 4) + 3)-(i * 4)) + 1;
uint8_t temp_39_rng_start = (i * 4);
for(temp_33_init = 0; temp_33_init < temp_34_rnge_size; temp_33_init++, temp_39_rng_start++){
state[temp_39_rng_start] = temp_23_rnge[temp_33_init];
}
i = (i + 1);
} 
}
